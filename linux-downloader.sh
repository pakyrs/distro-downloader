...
wget '$arch$release/*.iso' -P $download_location/
wget -r "*.iso" $arch/$release/ -P $download_location/


#Ubuntu 
ubuntu=ftp://releases.ubuntu.com/releases/
release=`curl -l $ubuntu | sort -n -r | awk NR==1`
wget -r -nH --cut-dirs=3 --no-parent -A "*.iso" $ubuntu/$release/ -P $torrent_location/
